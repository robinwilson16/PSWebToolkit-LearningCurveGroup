...
Partial Class webcontrols_searchnew
    ...
    ''' <summary>
    ''' Set true to Show Academic Year filter.
    ''' </summary>
    Private ShowAcademicYear As Boolean = True
    ''' <summary>
    ...
    Private Sub PopulateDropDownLists()
        ...
        '... or alteratvively hide the search fields that aren't being used

        'Get academic year based on today's date
        Dim currentAcademicYearID As String = AcademicYearHelper.GetCurrentAcademicYear()
        Dim nextAcademicYearID As String = AcademicYearHelper.GetNextAcademicYear()

        If ShowAcademicYear Then
            cboAcademicYear.Items.Add(New WebControls.ListItem("All", ""))
            'NOTE: PopulateListWithDescriptionColumns is a v simple way of using CCCListControlLibrary
            CCCListControlLibrary.PopulateListWithDescriptionColumns(New AcademicYearDataTable, cboAcademicYear, True, "")

            'Set selected academic year to next academic year if available, otherwise current academic year
            If cboAcademicYear.Items.FindByValue(nextAcademicYearID) IsNot Nothing Then
                cboAcademicYear.SelectedValue = nextAcademicYearID
            ElseIf cboAcademicYear.Items.FindByValue(currentAcademicYearID) IsNot Nothing Then
                cboAcademicYear.SelectedValue = currentAcademicYearID
            End If
        Else
            SearchAreaForAcademicYear.Visible = False
        End If
...
#Region " Grid "
    ...
    Sub LoadGridData()
        ...
        'End If
        If Not CCCBlankLibrary.IsBlank(cboAcademicYear.SelectedValue) Then
            Dim str = "EXISTS(SELECT * FROM Offering O WHERE O.CourseInformationID = CourseInformation.CourseInformationID " &
                "AND O.AcademicYearID = '" & cboAcademicYear.SelectedValue & "'" &
                ")"
    ...
    Private Sub m_DataPager_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles dpSearchGrid.PreRender
        If ViewState("Searched") = True AndAlso Not m_blnPageChanged Then
            ...
            dpSearchGrid.SetPageProperties(0, 50, True)
        End If
    End Sub
    ...
    Private Sub SetWebSiteAvailabilityIcon(ByVal img As HyperLink, ByVal rowView As System.Data.DataRowView)
        ...
        If m_CoursesInBasket.Contains(intCourseInfoID) Then
            ...
            imageText = "<h3 class=""text-secondary""><i class=""fa-solid fa-basket-shopping""></i></h3>"
        ...
        ElseIf CCCBlankLibrary.IsBlank(dataValue) Then
            imageText = "<h3 class=""text-danger""><i class=""fa-solid fa-circle-xmark""></i></h3>"
        ...
        Else
            ...
            If row IsNot Nothing Then
                ...
                Else
                    Select Case row.IconFileName.ToLower()
                        'Use Font Awesome icons instead of images
                        Case "icon_unavailable"
                            imageText = "<h3 class=""text-warning""><i class=""fa-solid fa-hourglass-half""></i></h3>"
                        Case "icon_enquirenow"
                            imageText = "<h3 class=""text-info""><i class=""fa-solid fa-circle-info""></i></h3>"
                        Case "icon_applynow"
                            imageText = "<h3 class=""text-success""><i class=""fa-solid fa-file-pen""></i></h3>"
                        Case "icon_enrolnow"
                            imageText = "<h3><i class=""fa-solid fa-graduation-cap""></i></h3>"
                        Case Else
                            imageText = "<h3><i class=""fa-solid fa-graduation-cap""></i></h3>"
                    End Select
